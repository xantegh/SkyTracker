...
var MilAircraftTypes = ['B52','C130','C30J','C17','C5M','C27J','K35R','K35E','C135','E6','E3CF','E3TF', 'P3','H60'];
	...
	for (var j=0; j < acs.length; j++) {
		...
		if (Planes[hex]) {
			...
			plane = new PlaneObject(hex);
                        ...
                        /*
			# Commented by Antoine to disable click and dblclick
...
function fetchData() {
	...
	FetchPending = $.ajax({ url: '/dump1090-fa/data/aircraft.json',
                                timeout: 5000,
                                cache: false,
                                dataType: 'json' });
...
function initialize() {
        ...
        // with initialization
        $.ajax({ url: '/dump1090-fa/data/receiver.json',
                 timeout: 5000,
                 cache: false,
                 dataType: 'json' })
...
function load_history_item(i) {
        ...
        $.ajax({ url: '/dump1090-fa/data/history_' + i + '.json',
                 timeout: 5000,
                 cache: false,
                 dataType: 'json' })
...
function initialize_map() {
	...
	if (SitePosition) {
        ...
        var request = $.ajax({ url: '/dump1090-fa/upintheair.json',
                               timeout: 5000,
                               cache: true,
                               dataType: 'json' });
...
function setColumnVisibility() {
    ...
    showColumn(infoTable, "#lat", false);
    showColumn(infoTable, "#lon", false);
    showColumn(infoTable, "#data_source", !mapIsVisible);
    showColumn(infoTable, "#airframes_mode_s_link", false);
    showColumn(infoTable, "#flightaware_mode_s_link", false);
    showColumn(infoTable, "#flightaware_photo_link", false);
}
